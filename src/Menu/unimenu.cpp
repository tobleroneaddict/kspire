...
void Menu::add_item(std::string label, std::function<void(Menu_Item *)> callback, int x_offset, int id)
{
    Menu_Item item;
    item.label = label;
    item.id = id;
...
void Menu::Update()
{
    ...
    for (unsigned int i = 0; i < items.size(); i++)
    {
        ...
        calc_y += 16 * i;
        if (calc_y >= text_cutoff_y)
            continue;
        if (i + scroll >= items.size())
            continue;

        uint16_t calc_color = 0xFFFF;
        if ((int)i == select_base)
            calc_color = colorRGB(0.3f, 1, 0.3f);

        fonts->drawString(items[i + scroll].label.c_str(), calc_color, *screen, calc_x, calc_y);
        fonts->drawString(items[i + scroll].value.c_str(), calc_color, *screen, val_calc_x + items[i + scroll].x_offset,
                          calc_y);
    }
    ...
    if (!lock_inputs)
    {
        ...
        if (!isKeyPressed(K_ENTER) && !isKeyPressed(K_PAD_N) && !isKeyPressed(K_PAD_S) && !isKeyPressed(K_PAD_W) &&
            !isKeyPressed(K_PAD_E))
            buttons_wiped = true;

        // if (scroll < 0) scroll = 0;
        if (scroll < -select_base)
            scroll = -select_base;
        if (scroll > ((int)items.size() - select_base - 1))
            scroll = ((int)items.size() - select_base - 1);
...
void Menu::render_frame()
{
    ...
    if (centered_to_screen)
    {
        if (show_title_bar)
        {
            drawTexture(texture_set.tex, *screen, 0, 0, 200, 150, (SCREEN_WIDTH - w) / 2, (SCREEN_HEIGHT - h + 24) / 2,
                        w, h - 8);
            // Titlebar
            drawTexture(texture_set.tex, *screen, 0, 0, 200, 3, (SCREEN_WIDTH - w) / 2, (SCREEN_HEIGHT - h) / 2, w, 16);
        }
        else
        {
            drawTexture(texture_set.tex, *screen, 0, 0, 200, 150, (SCREEN_WIDTH - w) / 2, (SCREEN_HEIGHT - h) / 2, w,
                        h);
        }
    }
    else
    ...
    if (show_title_bar)
    {
        if (centered_to_screen)
        {
            ...
            {
                fonts->drawStringCenter(titlebar.c_str(), 0xFFFF, *screen, ((SCREEN_WIDTH - w) / 2) + (w / 2),
                                        (SCREEN_HEIGHT - h + 1) / 2);
            }
            else
            {
                fonts->drawString(titlebar.c_str(), 0xFFFF, *screen, (SCREEN_WIDTH - w + 16) / 2,
                                  (SCREEN_HEIGHT - h + 1) / 2);
            }
        }
        else
        {
            if (titlebar_centered)
            {
                fonts->drawStringCenter(titlebar.c_str(), 0xFFFF, *screen, x + (w / 2), y + 3);
            }
            else
            {
                fonts->drawString(titlebar.c_str(), 0xFFFF, *screen, x + 8, y + 3);
            }
        }
    }
}