...
int VAB::Start(Bundle *assets)
{
    ...
    if (!hide_vab)
    {
        if (me.load_group(assets, "resources/vab/vab"))
            return 1;
    ...
    // Node
    if (node_g.load_group(assets, "resources/vab/node"))
        return 1;
...
void VAB::onClick_oneshot()
{
    ...
    if (show_pallete && kspire_pad.x_screen > 20)
    {
        ...
        { // Holding part?
            ...
            vessel.part_tree.erase(vessel.part_tree.begin() +
                                   grabbed_part); // Need to make this work with a whole subtree
        }
        else
        { // Create new part
            // Get the Part ID
            ...
            if ((unsigned int)page_index < parts_master->list_categories.size() && page_index >= 0)
            {
                ...
                if ((unsigned int)part_sel_index < cat_self_list.size())
                {
                    ...
                    part.attached = false;
                    if (vessel.part_tree.size() == 0)
                        part.attached = true; // Root part?
    ...
    { // TLDR: Grab the part
        ...
        for (size_t i = 0; i < vessel.part_tree.size(); ++i)
        {
            ...
            if (off < part_raycast_threshold)
            {
                ...
                // Super cool algorithm that finds the other node, and takes this node, and sets it to DETACHED magic
                // val
        ...
        if (has_grabbed_part)
        {
            for (Part &p : vessel.part_tree)
            {
                ...
                { // Client nodes
                    ...
                    for (Node &n_2 : vessel.part_tree[grabbed_part].nodes)
                    { // Host node
...
void VAB::Update()
{
    ...
    if (tsx < 16 && !show_pallete)
        show_pallete = true;
    if (tsx > 115 && show_pallete)
        show_pallete = false;
    if (m_moved && tsx < 16)
    {
        int num = tsy - 9;
    ...
    if (m_moved && tsx > 16 && tsx < 115 && tsy < 200)
    {
        ...
        if ((unsigned int)page_index < parts_master->list_categories.size() && page_index >= 0)
            max_part_index =
                (int)parts_master->get_parts_of_category(parts_master->list_categories[page_index]).size() - 1;
        if (max_part_index < 0)
            max_part_index = 0;
    ...
    if (!has_grabbed_part && !stopped_grabbing)
        cam.camera_controller(Camera::FREE);
    if (!kspire_pad.pressed)
    {
        pad_held = false;
        stopped_grabbing = false;
    } // release
...
void VAB::render()
{
    ...
    {
        if (parts_master->get_part_by_id(p.shared_id) == nullptr)
            continue;
        if (parts_master->get_part_by_id(p.shared_id)->models.size() == 0)
            continue;
        ...
        // There IS a bug here, if you look in the editor the current part will likely be highlit but theres nothing you
        // can really do ig
        ...
        if (has_grabbed_part /* && grabbed_part == &p*/)
        {
            for (Node &n : p.nodes)
            {
                ...
                glTranslatef(n.position.x * 2.2f, n.position.y * 2.2f, n.position.z * 2.2f);
