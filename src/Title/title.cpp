...
void Title::load_title(Bundle *resources, ngl_object *_obj, ngl_object *_moon)
{
    ...
    select_box.centered_to_screen = true;
    select_box.titlebar_centered = true;
    select_box.select_base = 0;
    select_box.allow_scroll_jump = false;
...
int Title::Update()
{
    ...
    if (!submenu_open)
    { // Cannot access top menu while in settings!
        ...
        if (select_index < 0)
            select_index = 4 + select_index; // Loop
    ...
    {
    case 0:
        drawTexture(texture_set.tex, *screen, 0, 249, 149, 23, SCREEN_WIDTH - 149, SCREEN_HEIGHT - 75, 149, 23);
        break;
    case 1:
        drawTexture(texture_set.tex, *screen, 0, 249, 149, 23, SCREEN_WIDTH - 149, SCREEN_HEIGHT - 49, 149, 23);
        break;
    case 2:
        drawTexture(texture_set.tex, *screen, 0, 275, 89, 23, SCREEN_WIDTH - 149, SCREEN_HEIGHT - 23, 89, 23);
        break;
    case 3:
        drawTexture(texture_set.tex, *screen, 92, 275, 57, 23, SCREEN_WIDTH - 58, SCREEN_HEIGHT - 23, 57, 23);
        break;
    default:
        break;
    }
    ...
    if (!isKeyPressed(K_PAD_N) && !isKeyPressed(K_PAD_S) && !isKeyPressed(K_ESC) && !isKeyPressed(K_ENTER))
        buttons_wiped = true;
...
// why the fuck is this necessary
#ifdef _TINSPIRE
        nglRotateY(fmod(fmod((off + 100) / 1, 360.0f) + 360.0f, 360.0f)); // Orbit
#else
        ...
        nglDrawArray(moon->vertices, moon->count_vertices, moon->positions, moon->count_positions, processed,
                     moon->draw_mode);
...
void Title::setup_settings()
{
    ...
    submenu.items.back().value = "";
    submenu.add_item("RESET CONTROLS TO DEFAULTS\n", [this](Menu_Item *item) { settings_onclick_action(item); }, 0, 15);
    submenu.items.back().value = "";
    ...
    submenu.items.back().value = "";
    submenu.add_item("RESET ALL TO DEFAULTS\n", [this](Menu_Item *item) { settings_onclick_action(item); }, 0, 34);
    submenu.items.back().value = "";
}
...
void Title::set_key_config(int key)
{
    ...
    {
        if (mi.id == menu_clicked_key_id)
            mi.value = std::to_string(key);
    }
