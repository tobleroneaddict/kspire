...
{
    if (s == "MOD_Engine")
        return ModTypes::MOD_Engine;
    if (s == "MOD_Command")
        return ModTypes::MOD_Command;
    if (s == "MOD_Tank")
        return ModTypes::MOD_Tank;
    if (s == "MOD_Decoupler")
        return ModTypes::MOD_Decoupler;
    if (s == "MOD_Strut")
        return ModTypes::MOD_Strut;
    if (s == "MOD_Torque")
        return ModTypes::MOD_Torque;
    return MOD_Command;
};
...
{
    std::string ICON;
    if (d.HasMember("staging_icon") && d["staging_icon"].IsString())
        ICON = d["staging_icon"].GetString();
    ...
    if (d.HasMember("id") && d["id"].IsInt())
        part->shared_id = d["id"].GetInt();

    if (d.HasMember("mass") && d["mass"].IsNumber())
        part->mass = d["mass"].GetFloat();
    if (d.HasMember("drag_min") && d["drag_min"].IsNumber())
        part->drag_min = d["drag_min"].GetFloat();
    if (d.HasMember("drag_max") && d["drag_max"].IsNumber())
        part->drag_max = d["drag_max"].GetFloat();
    if (d.HasMember("drag_ang") && d["drag_ang"].IsNumber())
        part->drag_ang = d["drag_ang"].GetFloat();
    if (d.HasMember("temp_max") && d["temp_max"].IsNumber())
        part->temp_max = d["temp_max"].GetFloat();
    if (d.HasMember("buoyancy") && d["buoyancy"].IsNumber())
        part->buoyancy = d["buoyancy"].GetFloat();
    ...
    if (d.HasMember("Nodes") && d["Nodes"].IsArray())
    {
        ...
        for (rapidjson::SizeType i = 0; i < nodes.Size(); i++)
        {
            ...
            if (this_node.HasMember("size") && this_node["size"].IsNumber())
                new_node.size = this_node["size"].GetInt();
            if (this_node.HasMember("crossfeed") && this_node["crossfeed"].IsNumber())
                new_node.crossfeed = this_node["crossfeed"].GetInt();
            if (this_node.HasMember("rigid_attach") && this_node["rigid_attach"].IsNumber())
                new_node.rigid_attach = this_node["rigid_attach"].GetInt();
    ...
    if (d.HasMember("Modules") && d["Modules"].IsArray())
    {
        ...
        for (rapidjson::SizeType i = 0; i < modules.Size(); i++)
        {
            ...
            if (mod_one.HasMember("thrust") && mod_one["thrust"].IsNumber())
                new_mod.thrust = mod_one["thrust"].GetFloat();
            if (mod_one.HasMember("torque_p") && mod_one["torque_p"].IsNumber())
                new_mod.torque_p = mod_one["torque_p"].GetFloat();
            if (mod_one.HasMember("torque_y") && mod_one["torque_y"].IsNumber())
                new_mod.torque_y = mod_one["torque_y"].GetFloat();
            if (mod_one.HasMember("torque_r") && mod_one["torque_r"].IsNumber())
                new_mod.torque_r = mod_one["torque_r"].GetFloat();
            if (mod_one.HasMember("crew_max") && mod_one["crew_max"].IsInt())
                new_mod.crew_max = mod_one["crew_max"].GetInt();
            if (mod_one.HasMember("crew_min") && mod_one["crew_min"].IsInt())
                new_mod.crew_min = mod_one["crew_min"].GetInt();
            ...
            if (mod_one.HasMember("Resources") && mod_one["Resources"].IsArray())
            {
                ...
                for (rapidjson::SizeType j = 0; j < mod_res.Size(); j++)
                {
                    ...
                    if (res.HasMember("name") && res["name"].IsString())
                        new_res.name = res["name"].GetString();
                    if (res.HasMember("rate") && res["rate"].IsNumber())
                        new_res.rate = res["rate"].GetFloat();
    ...
    if (d.HasMember("Resources") && d["Resources"].IsArray())
    {
        ...
        for (rapidjson::SizeType i = 0; i < resources.Size(); i++)
        {
            ...
            if (res.HasMember("name") && res["name"].IsString())
                new_res.name = res["name"].GetString();
            if (res.HasMember("amount") && res["amount"].IsNumber())
                new_res.rate = res["amount"].GetFloat();
            if (res.HasMember("amount_max") && res["amount_max"].IsNumber())
                new_res.rate = res["amount_max"].GetFloat();
...
int PartLoader::load_parts(Bundle *parts)
{
    ...
    for (ProtoPart &p : raw_parts)
    {
        ...
        if (d.HasMember("name") && d["name"].IsString())
            p.editor_name = d["name"].GetString();
        if (d.HasMember("description") && d["description"].IsString())
            p.editor_description = d["description"].GetString();
        if (d.HasMember("category") && d["category"].IsString())
            p.editor_category = d["category"].GetString();
        if (d.HasMember("model") && d["model"].IsString())
            p.objname = d["model"].GetString();
...
std::vector<int> PartLoader::get_parts_of_category(std::string category)
{
    ...
    for (ProtoPart &p : raw_parts)
    {
        if (p.editor_category == category)
            res.emplace_back(p.shared_id);
    }
    return res;
}
