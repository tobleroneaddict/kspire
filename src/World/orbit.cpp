...
void Orbit::calculate_state_from_keplers(double _UNIVERSAL_TIME)
{
    ...
    { // SOLVE ELLIPTICAL / CIRCULAR
        ...
        { // Noncircular orbits
            ...
            // Calc TA
            true_anomaly = linalg::atan2(linalg::sqrt(1 - eccentricity * eccentricity) * linalg::sin(eccentric_anomaly),
                                         linalg::cos(eccentric_anomaly) - eccentricity);
        }

        radius =
            semi_major_axis * (1.0 - eccentricity * eccentricity) / (1.0 + eccentricity * linalg::cos(true_anomaly));
    }
    ...
    {                            // SOLVE HYPERBOLIC
        ...
        true_anomaly = linalg::atan2(linalg::sqrt(eccentricity * eccentricity - 1.0) * linalg::sinh(eccentric_anomaly),
                                     eccentricity - linalg::cosh(eccentric_anomaly));

        radius = (0.0 - semi_major_axis) * (eccentricity * eccentricity - 1.0) /
    ...
    // Same mat as
    // https://github.com/Keeby-Astro/Space-Sciences-and-Astrodynamics/blob/main/course_scripts/Coordinate%20Transformation.py#L57
...
void Orbit::physics_to_rails(double epoch)
{
    ...
    linalg::vec<double, 3> e = (((VEL_mag * VEL_mag - mu / POS_mag) * POS) - (linalg::dot(POS, VEL) * VEL)) / mu;
