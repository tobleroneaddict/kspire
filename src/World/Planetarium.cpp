...
void Planetarium::update_planet_lighting(bool called_from_menu, float menu_angle)
{
    ...
    working_body++;
    if (working_body >= celestials.size())
        working_body = 1; // Reset
    ...
    if (c->me != nullptr)
    {
        ...
        // radians *= M_PI / 180.0f;
        if (called_from_menu)
            radians += (menu_angle + 240.0) * M_PI / 180.0f;
        ...
        // Sun rotation
        linalg::vec<float, 3> sun_dir = {_sun_dir.x * cosine + _sun_dir.y * sine,
                                         -_sun_dir.x * sine + _sun_dir.y * cosine, _sun_dir.z};
        ...
        for (unsigned int i = 0; i < ref->count_vertices; i++)
        {
            ...
            linalg::vec<float, 3> vertex_model = {(float)p->x, (float)p->z, (float)p->y};
            ...
            // Otherwise read
            auto normal = linalg::vec<float, 3>(v->normal.x, v->normal.y, v->normal.z);
...
void Planetarium::render_celestials(float fixed_bubble, bool map_mode, linalg::vec<double, 3> cam_pos)
{
    ...
    {
        printf("E 218754: No Focused Vessel!\n");
        return;
    }
    if (celestials.size() == 0)
    {
        printf("E 128585: Cannot render planets!\n");
        return;
    }
    ...
    for (CelestialBody &c : celestials)
    {
        ...
        else
        {
            ...
            if (obj != nullptr)
            {
                ...
                glTranslatef(-(v_x / len) * fixed_bubble * 1, -(v_y / len) * fixed_bubble * 1,
...
int Planetarium::load_celestial_bodies(Bundle *resources)
{
    ...
    for (rapidjson::SizeType i = 0; i < bodies.Size(); i++)
    {
        ...
        // Check
        if (s_name == nullptr || s_parent == nullptr || s_radius == nullptr || s_mass == nullptr || s_rr == nullptr ||
            s_ha == nullptr || s_atmh == nullptr || s_slp == nullptr)
        {
            printf("E 15123: Error parsing object!\n");
            return 1;
        }
        if (s_mu == nullptr || s_sma == nullptr || s_ecc == nullptr || s_inc == nullptr || s_lan == nullptr ||
            s_ap == nullptr || s_ma == nullptr || s_ep == nullptr)
        {
