...
void Universe::render_nearby_vessels()
{
    for (Vessel &v : vessels)
    {
        ...
        for (Part &p : v.part_tree) // Loop through AnGL scene
        {
            ...
            // Offset by part pos
            glTranslatef(p.pos.x + p.attPos.x, p.pos.y + p.attPos.y, p.pos.z + p.attPos.z);
...
void Universe::step_physics_orbit_for_v(Vessel *v)
{
    ...
    if (v->last_home_body != v->home_body)
        printf("SWITCH TO %d\n", v->home_body);
...
void Universe::Update()
{
    ...
    if (!isKeyPressed(K_MAP))
        map_button_held = false;
...
void Universe::render_map()
{
    ...
    linalg::vec<double, 3> pos_d(cam.pos.x, cam.pos.y, cam.pos.z);
...
void Universe::render_flight()
{
    ...
    glColor3f(intensity * ((float)0x87 / (float)0xFF), intensity * ((float)0xCE / (float)0xFF),
